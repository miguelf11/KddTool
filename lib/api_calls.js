if (Meteor.isServer) {
	Meteor.methods({
		...
		test: function (dataset_address) {

			var result1 = HTTP.call("POST", "http://localhost:8047/query.json",{ 
				data: {
					"queryType" : "SQL",
					"query" : "alter session set `store.format`='parquet'" //el que ellos necesiten
				}
			});

			if(result1.data.rows[0].ok == 'true'){
				var queryCopy = "create table dfs.root.`/user/hadoop/test/` as select * from dfs.`"+dataset_address+"`";
				console.log(queryCopy);
				try {
					var result = HTTP.call("POST", "http://localhost:8047/query.json",{ 
						data: {
