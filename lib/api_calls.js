if (Meteor.isServer) {
	Meteor.methods({
		...
		createHdfsFolderProject: function (userEmail,projectName) {
			...
			// this.unblock();
			try {
				var result = HTTP.call("PUT", "http://"+hadoop_host+"/webhdfs/v1"+cluster_root+"/projects/"+userEmail+"/"+projectName+"?op=MKDIRS&permission=777&user.name=hadoop");
				return result;
		...
		removeHdfsFolder: function (hdfs_address) {
			...
			// this.unblock();
			try {
				var result = HTTP.call("DELETE", "http://"+hadoop_host+"/webhdfs/v1"+hdfs_address+"?op=DELETE&recursive=true&user.name=hadoop");
				console.log(result);
				return result;
			} catch (e) {
				// Got a network error, time-out or HTTP error in the 400 or 500 range.
        ...
        queryDataDrill: function (data_address){
            console.log('drill');
            //En query colocamos el query que queremos realizar sobre el .csv en el cluster
            var queryCopy = "select * from dfs.`"+data_address+"/raw/*` limit 30";
        ...
        queryDatasetRows: function (data_address){
            // console.log('drill');
            //En query colocamos el query que queremos realizar sobre el .csv en el cluster
            var queryCopy = "select count(*) from dfs.`"+data_address+"` limit 1";
            console.log(queryCopy);
            try {
                var result = HTTP.call("POST", "http://localhost:8047/query.json",{ data: {
                        "queryType" : "SQL",
