...
if (Meteor.isServer) {
	Meteor.methods({
        ...
        queryDataDrillMaxValue: function (field,data_type,data_address){
            // console.log('drill');
            var queryCopy = "select max(cast("+field+" as "+data_type+")) from dfs.`"+data_address+"/raw/*` limit 1";
            console.log(queryCopy);
            try {
                var result = HTTP.call("POST", "http://localhost:8047/query.json",{ data: {
                        "queryType" : "SQL",
        ...
        queryDataDrillMinValue: function (field,data_type,data_address){
            // console.log('drill');
            var queryCopy = "select min(cast("+field+" as "+data_type+")) from dfs.`"+data_address+"/raw/*` limit 1";
            console.log(queryCopy);
            try {
                var result = HTTP.call("POST", "http://localhost:8047/query.json",{ data: {
                        "queryType" : "SQL",
        ...
        queryDataDrillAVGValue: function (field,data_type,data_address){
            // console.log('drill');
            var queryCopy = "select avg(cast("+field+" as "+data_type+")) from dfs.`"+data_address+"/raw/*` limit 1";
            console.log(queryCopy);
            try {
                var result = HTTP.call("POST", "http://localhost:8047/query.json",{ data: {
                        "queryType" : "SQL",
        ...
        queryDataDrillOrderBy: function (column,data_type,order,data_address){
            // console.log('drill');
            var queryCopy = "select * from dfs.`"+data_address+"/raw/*` order by (cast("+column+" as "+data_type+")) "+order+" limit 100";
            console.log(queryCopy);
            try {
                var result = HTTP.call("POST", "http://localhost:8047/query.json",{ data: {
                        "queryType" : "SQL",
            ...
            try {
                ...
                if (result0.data.rows[0].ok == 'true'){
                	...
                	if(result1.data.rows[0].ok == 'true'){
                		...
                		var queryCopy = "create table dfs.root.`"+folder_project+"/raw` as select *,row_number() over(partition by 1) as `drill_row_num` from dfs.`"+dataset_address+"`";
                		console.log(queryCopy);
        ...
        queryPrueba: function (){
            var queryCopy = "SELECT * FROM dfs.`/user/vit/datasets/iris.csv` LIMIT 1";
            console.log(queryCopy);
            try {
                var result = HTTP.call("POST", "http://localhost:8047/query.json",{ data: {
                        "queryType" : "SQL",
                ...
                console.log("result.data.columns "+result.data.columns);
                return result;
