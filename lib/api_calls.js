if (Meteor.isServer) {
	Meteor.methods({
		...
		// 	var queryCopy = "select * from dfs.`/user/hadoop/projects/admin@admin.com/miproyecto1492351932623/raw/0_0_0.parquet`";
		...
		// 	string(result.data.rows, {
		// 		delimiter: ',',
		// 		header: true,
		// 		columns: result.data.columns
		// 	}, Meteor.bindEnvironment((err, data) => {

		// 		hdfs.writeFile("/user/hadoop/tmp/0_0_0.csv", data, Meteor.bindEnvironment((err, res) =>{
		// 			if (err) {
		// 				console.log(err);
		// 			}else{
		// 				console.log("Todo bien");
		// 				queryCopy = "create table dfs.root.`/user/hadoop/test` as  select * from dfs.`/user/hadoop/tmp/0_0_0.csv`";

		// 				HTTP.call("POST", "http://localhost:8047/query.json",{ 
		// 					data: {
		// 						"queryType" : "SQL",
		// 						"query" : queryCopy
		// 					}
		...
		replaceValues: (new_version_address,old_version_address, column, initVal, postVal, order) => {
			try	{
				...
				var res = HTTP.call("PUT", "http://"+hadoop_host+"/webhdfs/v1"+cluster_root+"/tmp/0_0_0.json?op=CREATE&overwrite=true&user.name="+cluster_user);
				res = HTTP.call("PUT", res.headers.location);
				res = HTTP.call("POST", "http://"+hadoop_host+"/webhdfs/v1"+cluster_root+"/tmp/0_0_0.json?op=APPEND&user.name="+cluster_user);
				res = HTTP.call("POST", res.headers.location, {data: rows});
