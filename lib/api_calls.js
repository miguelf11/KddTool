if (Meteor.isServer) {
	Meteor.methods({
        ...
        queryDataDrillOrderBy: function (column,data_type,order,data_address){
            // console.log('drill');
            if (data_type == 'DOUBLE' || data_type == 'BIGINT'){
                var queryCopy = "select * from dfs.`"+data_address+"/*` where "+column+" <> '' order by (cast("+column+" as "+data_type+")) "+order+" limit 100";  
            }else{
                var queryCopy = "select * from dfs.`"+data_address+"/*` order by (cast("+column+" as "+data_type+")) "+order+" limit 100";
            }
        ...
        //     });
        //     console.log('inicio');
        //     var result = HTTP.call("GET", "http://"+hadoop_host+"/webhdfs/v1"+cluster_root+"/datasets/train1491868573478.csv?op=OPEN&user.name="+cluster_user);
        ...
        //     // hdfs.createWriteStream('/user/hadoop/datasets/train.csv');
        // },
    });
}